                ...
                <div class="col-sm-12">
                    <div class="card">
                        ...
                        <div class="card-block table-border-style">
                            <table class="table w-100" id="nota_dinas-table">
                                <thead>
                                    <tr>
                                        ...
                                        @if ( auth()->user()->hasPermission('report.detail_nota_dinas') )
...
<script>
    ...
    $(document).ready(function(){
        ...
        var barangMasukTable = $('#nota_dinas-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                data : [
            ...
            "createdRow": function(row, data){
                $(row).find(' button.detail-nota_dinas').click(function(e){
                    console.log($(this));
                    $.ajax({
                        url: "{{ url('nota_dinas/detail') }}/" + $(this).data('id'),
                        type: 'get',
                        dataType: 'html',
                        success: function(data){
                            $('.modal#detail-nota_dinas #modal-body').html(data);
                        }
                    });
                    $('.modal#detail-nota_dinas').modal('toggle');
                });
                console.log('created');
            }
        });
        ...
        $('#filtering-report').on('click', function(e){
            ...
            barangMasukTable.ajax.url("{{ url('report/nota_dinas') }}?startDate=" + startDate + "&endDate=" + endDate + "&id_gudang=" + id_gudang).load();
        });
        ...
        $('#reset-filtering').on('click', function(e){
            cb(start, end);
            barangMasukTable.ajax.url("{{ url('report/nota_dinas') }}").load();
        });
