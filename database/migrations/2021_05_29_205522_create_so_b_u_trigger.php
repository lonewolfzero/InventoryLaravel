...
class CreateSoBUTrigger extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::unprepared("DROP TRIGGER IF EXISTS `so_b_u`;
        CREATE TRIGGER so_b_u
        BEFORE UPDATE
        ...
        begin
            ...
            if extract(day from new.tanggal_pelaksanaan) <> extract(day from last_day(new.tanggal_pelaksanaan)) then
                SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Update gagal : Tanggal pelaksanaan harus di akhir bulan';
            end if;
            ...
            where 
                ...
                so.id_gudang = new.id_gudang and so.id <> new.id;
            if ada > 0 then
                ...
                set peringatan = concat_ws('', 'Update gagal : ', namagudang, ' pada bulan ', namabulan, ' sudah dilakukan stock opname');
